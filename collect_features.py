...
def get_webds_loader(dset_path):
    ...
    dataset = (wds.WebDataset(url).decode("pil").to_tuple("input.jpg", "output.cls", "info").map_tuple(preproc, identity))
...
def test(model, dset_path, file_name=None):
    ...
    info_file = open('data/meta_info.txt', 'w')

    for i, (images, labels, info) in enumerate(dataset_loader):
        images, labels = images.cuda(non_blocking=True), labels.cuda(non_blocking=True)
        features = model.encoder(images)
        output = model.classifier(features)
        
        for s in info:
            info_file.write(str(s)+'\n')
        
        # feature_dict = {'feature': features, 'info': info}


        # torch.save(features, f'data/src_features/{i}.pth')
